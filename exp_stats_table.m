function[stats_table] = exp_stats_table(portfolios, portfolio_index)
...
for x = 1:length(portfolios)
    data(1,x) = cellstr(strcat(num2str(round((portfolios(x).portexpTR * 1000))/10),' %'));
end
for x = 1:length(portfolios)
    data(2,x) = cellstr(strcat(num2str(round((portfolios(x).portexpER * 1000))/10),' %'));
 end
for x = 1:length(portfolios)
    data(3,x) = cellstr(strcat(num2str(round((portfolios(x).expvol * 1000))/10),' %'));
 end
for x = 1:length(portfolios)
    data(4,x) = cellstr(num2str(round(portfolios(x).portexpSR * 100)/100));
end
...
if portfolio_index ~= 0
    data = cell(4,1);
    data(1) = cellstr(strcat(num2str(round((portfolios(portfolio_index).portexpTR * 1000))/10),' %'));
    data(2) = cellstr(strcat(num2str(round((portfolios(portfolio_index).portexpER * 1000))/10),' %'));
    data(3) = cellstr(strcat(num2str(round((portfolios(portfolio_index).expvol * 1000))/10),' %'));
    data(4) = cellstr(num2str(round(portfolios(portfolio_index).portexpSR * 100)/100));
